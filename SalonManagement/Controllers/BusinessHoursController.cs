...
public class BusinessHoursController(ApplicationDbContext dbContext) : ControllerBase
{
    ...
    {
        var stored = await dbContext.BusinessHours.ToDictionaryAsync(hours => hours.DayOfWeek);
        ...
        }).OrderBy(item => item.DayOfWeek == DayOfWeek.Sunday ? 7 : (int)item.DayOfWeek));
    }

    [HttpPut]
    public async Task<IActionResult> Update(UpdateBusinessHoursRequest request)
    {
        if (request.Days.Select(day => day.DayOfWeek).Distinct().Count() != 7 ||
            request.Days.Any(day => day.DayOfWeek is < 0 or > 6))
        {
            ModelState.AddModelError(nameof(request.Days), "Cấu hình phải chứa đúng 7 ngày, không trùng lặp.");
        }
        ...
        foreach (var day in request.Days.Where(day => !day.IsClosed))
        {
            ...
            {
                ModelState.AddModelError($"Days[{day.DayOfWeek}]", "Ngày mở cửa phải có giờ mở và giờ đóng.");
            }
            else if (day.ClosesAt.Value.ToTimeSpan() - day.OpensAt.Value.ToTimeSpan() < TimeSpan.FromMinutes(60))
            {
                ModelState.AddModelError($"Days[{day.DayOfWeek}]", "Giờ đóng phải sau giờ mở ít nhất 60 phút.");
            }
        }
        ...
        foreach (var input in request.Days)
        {
            ...
            {
                hours = new BusinessHour { DayOfWeek = day };
